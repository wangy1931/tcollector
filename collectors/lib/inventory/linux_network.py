...
class LinuxNetwork:
    ...
    def populate(self, collected_facts=None):
        network_facts = {}
        ip_path = 'ip'  # self.module.get_bin_path('ip')
        if ip_path is None:
    ...
    def get_default_interfaces(self, ip_path, collected_facts=None):
        ...
        for v in 'v4', 'v6':
            ...
            if v == 'v6' and not socket.has_ipv6:
                continue
            try:
                out = subprocess.check_output(' '.join(command[v]), shell=True)
            except Exception as e:
                self.logger.warn('caught exception on running ip: %s' % e)
                continue
    ...
    def get_interfaces_info(self, ip_path, default_ipv4, default_ipv6):
        ...
        for path in glob.glob('/sys/class/net/*'):
            ...
            try:
                ...
                primary_data = subprocess.check_output(' '.join(args), shell=True)

                args = [ip_path, 'addr', 'show', 'secondary', device]
                secondary_data = subprocess.check_output(' '.join(args), shell=True)
    ...
    def get_ethtool_data(self, device):
        ...
        data = {}
        ethtool_path = 'ethtool' # self.module.get_bin_path("ethtool")
        ...
        if ethtool_path:
            ...
            try:
                ...
                self.logger.warn('caught exception on running ethtool: %s' % e)
                stdout = None
