...
def get_uname_version():
    try:
        out = subprocess.check_output('uname -v')
    except Exception:
        return None
    ...
    def parse_distribution_file_Debian(self, name, data, path, collected_facts):
        ...
        if 'Debian' in data or 'Raspbian' in data:
            ...
            if collected_facts['distribution_release'] == 'NA' and 'Debian' in data:
                dpkg_cmd = 'dpkg' # self.module.get_bin_path('dpkg')
                if dpkg_cmd:
...
class Distribution(object):
    ...
    def get_distribution_AIX(self):
        aix_facts = {}
        try:
            out = subprocess.check_output("/usr/bin/oslevel")
            data = out.split('.')
            aix_facts['distribution_version'] = data[0]
            aix_facts['distribution_release'] = data[1]
        except Exception as e:
            self.logger.warn('caught exception on running oslevel: %s' % e)
    ...
    def get_distribution_HPUX(self):
        hpux_facts = {}
        try:
            out = subprocess.check_output("/usr/sbin/swlist |egrep 'HPUX.*OE.*[AB].[0-9]+\.[0-9]+'", use_unsafe_shell=True)
            ...
            self.logger.warn('caught exception on running swlist: %s' % e)
    ...
    def get_distribution_Darwin(self):
        ...
        try:
            out = subprocess.check_output("/usr/bin/sw_vers -productVersion")
            data = out.split()[-1]
            darwin_facts['distribution_version'] = data
        except Exception as e:
            self.logger.warn('caught exception on running sw_vers: %s' % e)
    ...
    def get_distribution_OpenBSD(self):
        ...
        try:
            out = subprocess.check_output("/sbin/sysctl -n kern.version")
            ...
            self.logger.warn('caught exception on running sysctl: %s' % e)
