...
class WindowsIostats(CollectorBase):

    def __init__(self, config, logger, readq):
        super(WindowsIostats, self).__init__(config, logger, readq)

    def __call__(self):
        io = w32.IO(self._logger)
        config = dict( device_blacklist_re=None )
        metrics = io.check(config)
...
if __name__ == "__main__":
    iostats_inst = WindowsIostats(None, None, Queue())
    iostats_inst()
