...
try:
    options,args = getopt.getopt(sys.argv[1:],"", ["help","ORG_TOKEN=","CLIENT_ID=","METRIC_SERVER_HOST=","ALERTD_SERVER=","AGENT_URL="])
except Exception,e:
    print e.message
    sys.exit()
...
# CLIENT_ID=""
# set_client_id=edit_file('^client_id *=.*','client_id = %s'%CLIENT_ID,filenames=['uagent/uagent.conf'])
# message="failed to set client_id value in uagent/uagent.conf\n"
# abort_if_failed(set_client_id,message)
#
OS="windows"
set_os=edit_file
...
abort_if_failed(set_filebeat_token,message="",re='<token>',replace_string=ORG_TOKEN,filenames=['winlogbeat-5.4.3-windows-x86_64/winlogbeat.yml'])
LOG_SERVER_HOST_PORT=METRIC_SERVER_HOST+":5040"
set_filebeat_output_local=edit_file('<log-server-host-port>',LOG_SERVER_HOST_PORT,filenames=['winlogbeat-5.4.3-windows-x86_64/winlogbeat.yml'])
#
