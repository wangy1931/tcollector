...
Function unzip{
    ...
    if ( !(Test-Path \opt\cloudwiz-agent\filebeat)){
        run_cmd $root_dir "mklink /d filebeat filebeat-5.4.2-windows-x86_64"
    }
}
...
function start_all_default_collector(){
    ...
    cmd /c $cmd_str
     $collector_list="summary","win32_host_scan","service_scan","win32_cpustats","win32_dfstats","win32_top_n","win32_iostats","win32_memstats","win32_procstats","win32_events"
